...
export default function App() {
  ...
  const createNewProject = async () => {
    if (userTier === 'free' && projects.length >= 5) {
      if (window.confirm("Free tier is limited to 5 projects. Would you like to upgrade to Pro to create more?")) { window.open((userTier === 'free' ? 'https://kojilaunch.com/fe-monogram/' : 'https://kojilaunch.com/monogram-version/'), "_blank"); }
      return;
    }
    try {
  ...
  const processFiles = async (files: File[]) => {
    ...
    if (images.length + allowedFiles.length > maxPages) {
      if (window.confirm(`Your tier (${userTier}) is limited to ${maxPages} pages per book. Would you like to upgrade to add more?`)) { window.open((userTier === 'free' ? 'https://kojilaunch.com/fe-monogram/' : 'https://kojilaunch.com/monogram-version/'), "_blank"); }
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white border border-neutral-200 rounded-md shadow-lg py-1 z-50 overflow-hidden">
                  ...
                  {(images.length > 1 || view === 'bookflow') && (
                    <>
                      <button onClick={() => { 
                        setIsExportMenuOpen(false); 
                        if (userTier !== 'pro') { if (window.confirm("Export SVG (Full photo) is a Pro feature. Would you like to upgrade now?")) { window.open((userTier === 'free' ? 'https://kojilaunch.com/fe-monogram/' : 'https://kojilaunch.com/monogram-version/'), "_blank"); } return; }
                        handleBulkExportSVG(); 
                      ...
                      <button onClick={() => { 
                        setIsExportMenuOpen(false); 
                        if (userTier !== 'pro') { if (window.confirm("Export PNG (Full photo) is a Pro feature. Would you like to upgrade now?")) { window.open((userTier === 'free' ? 'https://kojilaunch.com/fe-monogram/' : 'https://kojilaunch.com/monogram-version/'), "_blank"); } return; }
                        handleBulkExportPNG(); 
...
<aside className="w-64 bg-white border-r border-neutral-200 p-3.5 flex flex-col gap-1.5 overflow-y-auto shrink-0 custom-scrollbar">
                    ...
                    <div className="flex bg-neutral-100/80 p-1 rounded-lg border border-neutral-200">
                      {['square', 'circle', 'triangle', 'hexagon', 'diamond'].map(shape => {
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (isLockedShape) {
                              if (window.confirm(`The ${shape} pixel shape is only available in the Pro version. Would you like to upgrade now?`)) { window.open((userTier === 'free' ? 'https://kojilaunch.com/fe-monogram/' : 'https://kojilaunch.com/monogram-version/'), "_blank"); }
                              return;
                            }
            ...
            <div className="mt-auto pt-4 pb-1">
              <div className="flex flex-col gap-2 p-3 bg-neutral-100/80 rounded-xl border border-neutral-200 shadow-sm">
                ...
                {userTier !== 'pro' && (
                  <button 
                    onClick={() => window.open((userTier === 'free' ? 'https://kojilaunch.com/fe-monogram/' : 'https://kojilaunch.com/monogram-version/'), '_blank')}
