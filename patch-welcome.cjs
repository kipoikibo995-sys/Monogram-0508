...
const regexReplace = /case 'welcome':\s*return <TextPage value=\{data\.welcomePage\} onChange=\{\(v\) => handleUpdateData\('welcomePage', v\)\} placeholder="Welcome, How to use this book, Pen recommendations\.\.\." \/>;/;
...
const WelcomePage = ({ value, onChange }: { value?: string; onChange: (v: string) => void }) => {
  ...
  return (
    <div className="flex-1 w-full h-full p-12 bg-white flex flex-col font-sans relative">
      <input
        value={parsedValue.title}
        onChange={(e) => handleChange('title', e.target.value)}
        className="text-[28px] font-bold text-black outline-none w-full bg-transparent hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors uppercase mb-8"
      />
      
      <textarea
        value={parsedValue.intro}
        onChange={(e) => handleChange('intro', e.target.value)}
        className="w-full resize-none outline-none bg-transparent text-[20px] leading-[1.6] hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors text-black mb-12"
        rows={3}
      />
      ...
      <div className="flex w-full gap-8">
        ...
        <div className="w-[45%] flex flex-col">
          <input
            ...
            className="text-[22px] font-bold text-black outline-none w-full bg-transparent hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors uppercase mb-6"
          />
          
          <textarea
            value={parsedValue.howToSteps}
            onChange={(e) => handleChange('howToSteps', e.target.value)}
            className="w-full resize-none outline-none bg-transparent text-[20px] leading-[1.6] hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors text-black mb-10"
            rows={8}
          />
          ...
          <input
            ...
            className="text-[22px] font-bold text-black outline-none w-full bg-transparent hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors uppercase mb-2"
          />
          
          <textarea
            value={parsedValue.penIntro}
            onChange={(e) => handleChange('penIntro', e.target.value)}
            className="w-full resize-none outline-none bg-transparent text-[20px] leading-[1.6] hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors text-black mb-4"
            rows={2}
          />
          ...
          <div className="flex flex-col gap-2 mb-6">
            {parsedValue.penList.map((item, idx) => (
              <div key={idx} className="flex items-start gap-2 group relative">
                <span className="text-[22px] text-slate-500 font-bold leading-none mt-1">✓</span>
                <input
                   value={item}
                   onChange={(e) => handleListChange(idx, e.target.value)}
                   className="flex-1 outline-none bg-transparent text-[20px] hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors text-black"
                />
                <button onClick={() => removeListItem(idx)} className="absolute -right-8 top-1 text-red-500 opacity-0 group-hover:opacity-100 transition-opacity print:hidden">
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
            <button onClick={addListItem} className="text-blue-500 text-sm text-left hover:underline print:hidden mt-2">
              + Add Item
            </button>
          </div>
          ...
          <textarea
            ...
            className="w-full resize-none outline-none bg-transparent text-[20px] leading-[1.6] hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors text-black"
            rows={8}
          />
        </div>
        ...
        <div className="w-[55%] flex flex-col pl-4">
          {/* Legend Table */}
          <div className="flex flex-col border border-neutral-100 rounded overflow-hidden mb-12 shadow-sm">
            {parsedValue.legend.map((row, i) => (
              <div key={i} className={\`flex items-center p-3 border-b border-neutral-100 last:border-b-0 \${i % 2 === 0 ? 'bg-neutral-50' : 'bg-white'}\`}>
                <div className="w-12 text-center font-bold text-lg text-black">{row.num}</div>
                ...
                <div className="w-16 h-12 flex items-center justify-center border border-neutral-300 bg-white shadow-sm mr-2 text-xl font-bold">
                   {row.symbol === '■' ? <div className="w-6 h-6 bg-black"></div> : (
                     <input 
                       value={row.symbol}
          ...
          {/* Illustration Area */}
          <div className="flex-1 flex items-end justify-center w-full min-h-[200px] relative group">
             {parsedValue.illustrationImage ? (
               <div className="relative w-full h-full flex items-center justify-center p-8">
                 ...
                 <button 
                    ...
                    className="absolute top-4 right-4 p-2 bg-white/90 hover:bg-red-50 text-neutral-600 hover:text-red-600 rounded-full shadow-lg transition-all opacity-0 group-hover:opacity-100 print:hidden"
                 >
                   <Trash2 size={20} />
                 </button>
               </div>
...
code = code.replace(regexReplace, "case 'welcome':\n        return <WelcomePage value={data.welcomePage} onChange={(v) => handleUpdateData('welcomePage', v)} />;");
code = code.replace('// --- TextPage Component ---', welcomeComponent + '\n// --- TextPage Component ---');
