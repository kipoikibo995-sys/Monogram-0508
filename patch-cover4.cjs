...
const CoverPage = ({ value, onChange }: { value?: string; onChange: (v: string) => void }) => {
  ...
  try {
    if (value && value.startsWith('{')) {
      ...
      if (parsed.image && !parsed.images) {
        parsedValue.images = [parsed.image];
      }
    } else if (value) {
  ...
  if (parsedValue.images.length > 0) {
    const imgCount = parsedValue.images.length;
    return (
      <div 
        className={\`flex-1 w-full h-full relative group bg-white \${isDragging ? 'opacity-80' : ''}\`}
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-between h-full bg-white relative font-sans w-full p-2 overflow-hidden">
       ...
       >
          <div 
            onClick={() => fileInputRef.current?.click()}
            className={\`w-3/4 h-full border-4 border-dashed \${isDragging ? 'border-purple-500 bg-purple-50' : 'border-neutral-300 hover:border-neutral-400 hover:bg-neutral-50'} cursor-pointer flex flex-col items-center justify-center transition-colors rounded-2xl p-8 text-center\`}
          >
            ...
            <p className="text-neutral-500 text-base max-w-sm">
              Upload up to 4 images. When you upload, this template will be replaced by your custom layout.
            </p>
          </div>
          <input 
            type="file" 
