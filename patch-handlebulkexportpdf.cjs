...
const oldFunc = `  const handleBulkExportPDF = async () => {
    ...
    try {
      ...
      const addImageToPDF = (imgObj: {img: HTMLImageElement, settings: ImageSettings}, mode: 'workbook'|'solution', isFirst: boolean) => {
      ...
      for (let i = 0; i < images.length; i++) {
        addImageToPDF(images[i], 'workbook', i === 0);
...
const newFunc = `  const handleBulkExportPDF = async () => {
    ...
    try {
      ...
      const addDataUrlToPDF = (dataUrl: string, isFirst: boolean, margin: number = 0, gutter: number = 0) => {
        pageCount++;
        const isLeftPage = pageCount % 2 === 0;
        const leftMargin = isLeftPage ? margin : margin + gutter;
        const rightMargin = isLeftPage ? margin + gutter : margin;
...
if (code.includes('const handleBulkExportPDF = async () => {')) {
