...
// Also need to fix the syntax error const [mode, setMode] = useState<'', 'custom' | 'template'>('');
// It should be useState<'' | 'custom' | 'template'>('');
code = code.replace("useState<'', 'custom' | 'template'>('');", "useState<'' | 'custom' | 'template'>('');");
