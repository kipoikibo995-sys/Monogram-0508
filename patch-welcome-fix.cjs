...
const WelcomePage = ({ value, onChange }: { value?: string; onChange: (v: string) => void }) => {
  ...
  return (
    <div className="flex-1 w-full h-full p-10 bg-white flex flex-col font-sans relative">
      <input
        value={parsedValue.title}
        onChange={(e) => handleChange('title', e.target.value)}
        className="text-[26px] font-bold text-black outline-none w-full bg-transparent hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors uppercase mb-4"
      />
      
      <textarea
        value={parsedValue.intro}
        onChange={(e) => handleChange('intro', e.target.value)}
        className="w-full resize-none outline-none bg-transparent text-[18px] leading-[1.5] hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors text-black mb-6"
        rows={3}
      />
      ...
      <div className="flex w-full gap-8">
        ...
        <div className="w-[45%] flex flex-col">
          <input
            ...
            className="text-[20px] font-bold text-black outline-none w-full bg-transparent hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors uppercase mb-4"
          />
          ...
          <input
            ...
            className="text-[20px] font-bold text-black outline-none w-full bg-transparent hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors uppercase mb-2"
          />
          
          <textarea
            value={parsedValue.penIntro}
            onChange={(e) => handleChange('penIntro', e.target.value)}
            className="w-full resize-none outline-none bg-transparent text-[18px] leading-[1.5] hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors text-black mb-2"
            rows={2}
          />
          ...
          <div className="flex flex-col gap-1 mb-4">
            {parsedValue.penList.map((item, idx) => (
              <div key={idx} className="flex items-start gap-2 group relative">
                <span className="text-[20px] text-slate-500 font-bold leading-none mt-1">✓</span>
                <input
                   value={item}
            ...
            <button onClick={addListItem} className="text-blue-500 text-sm text-left hover:underline print:hidden mt-1">
              + Add Item
            </button>
          </div>
          ...
          <textarea
            ...
            className="w-full resize-none outline-none bg-transparent text-[18px] leading-[1.5] hover:bg-neutral-100 focus:bg-neutral-50 rounded transition-colors text-black"
            rows={6}
          />
        </div>
        
        {/* Right Column */}
        <div className="w-[55%] flex flex-col pl-2">
          {/* Legend Table */}
          <div className="flex flex-col border border-neutral-100 rounded overflow-hidden mb-6 shadow-sm">
          ...
          {/* Illustration Area */}
          <div className="flex-1 flex items-end justify-center w-full min-h-[180px] relative group">
             {parsedValue.illustrationImage ? (
               <div className="relative w-full h-full flex items-center justify-center p-4">
                 <img src={parsedValue.illustrationImage} alt="Illustration" className="max-w-[90%] max-h-[250px] object-contain mix-blend-multiply" />
                 <button 
